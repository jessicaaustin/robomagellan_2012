...
// 26 Jul 2012 added a sequence number to the Euler angle messages
//
...
#define MAIN_LOOP_TIME_MS 40 // the number of milliseconds between iterations
#define COMPASS_UPDATE_RATE 2 // only read the compass after this many passes
                              // through the main loop

#define ADC_WARM_CYCLES 50
#define STATUS_LED 5 // it doesn't seem to be either 5 or 13
...
{
  Serial.println();
  Serial.println("Sparkfun 9DOF Razor IMU v1.06.02");
  Serial.println("Attitude and Heading Reference System");
...
void loop() //Main Loop
{
  ...
  if((DIYmillis()-timer) >= MAIN_LOOP_TIME)  // Main loop rate
  {
    ...
    if (counter > COMPASS_UPDATE_RATE)  
      {
      counter=0;
    ...
    {
        //
        // If one or more characters are received on the serial port,
        // execute the initialization process again. This is used to
        // recalibrate the 9DOF, without physically disturbing it.
        //
